...
def _switchingEq(beta, x):
    ...
    mu, dG, fmax, fmin = _extractVariables(beta, x)
    B = np.exp(-(dG - mu)/RT)
    Q = B.sum(axis=0)
...
def _switchingEq_jacobian_mu(beta, x):
    """Compute the Jacobian of the switching equation with respect to mu"""
    mu, dG, fmax, fmin = _extractVariables(beta, x)
    B = np.exp(-(dG - mu)/RT)
    Q = B.sum(axis=0)
    Q_weighted = (B * (fmax - fmin)).sum(axis=0)
...
def _switchingEq_jacobian_x(beta, x):
    """Compute the Jacobian of the switching equation with respect to dG, fmax, and fmin"""
