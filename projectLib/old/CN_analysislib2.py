...
def normalize(data):
    data_norm = pd.DataFrame({'clusterID': data['clusterID'], 
                              ...
                              'switch1_norm': data['switch1'] / data['allCluster'], 
                              'switch2_norm': data['switch2'] / data['allCluster'], 
                              'diff1': (data['switch1'] - data['quenched']) / (data['allCluster'] - data['quenched']), 
                              'diff2': (data['switch2'] - data['quenched']) / (data['allCluster'] - data['quenched'])
                              }) 
    return data_norm
...
#def summarize(data, numCores, numSamples, alpha):
    ...
    data_agg = data_norm_grouped['allCluster', 'quenched', 'switch1', 'switch2', 
                                 'quenched_norm', 'switch1_norm', 'switch2_norm', 'diff1', 'diff2'].agg(['size', 'median', 'sem'])
...
def filter(data_agg, numClusterCutoff): 
    data_aggPF = data_agg[data_agg['switch2']['size'] > numClusterCutoff]
    return data_aggPF
